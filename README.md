...
รองรับทั้งเมาส์คีย์บอร์ดและการแตะบนแท็บเล็ต (มีปุ่มทิศทางบนจอให้ในโซนซาฟารี)

มีระบบบัญชีผู้เล่น (หลายคนเล่นในเครื่องเดียวได้ แยกเซฟกัน) และหน้าผู้ดูแลสำหรับปรับค่าเกม

ไม่ได้ซื้อโปเกม่อนด้วยเงิน — ต้อง **ออกไปจับเอง** ที่โซนซาฟารี แล้วพกลงด่านได้ **สูงสุด 6 ตัว**
ตามกฎจริงของโปเกม่อน มี **Gen 1-3 ครบ 386 ตัว** (คันโต โจโต โฮเอ็น)
ใช้สไปรท์จริงแบบเคลื่อนไหวชุด Black/White พร้อม **เมก้าอีโวลูชัน 43 ร่าง**
และ **เควสพิเศษสำหรับโปเกม่อนในตำนานครบทั้ง 21 ตัว**
...
การเล่นด้วยการแตะรองรับครบ ไม่ต้องใช้คีย์บอร์ดเลย:

| อยากทำอะไร | บนจอสัมผัส | บนคอม |
|---|---|---|
| เดินในซาฟารี | **ปุ่มทิศทางมุมล่างขวา** (แตะทีละก้าว หรือกดค้างเดินยาว) หรือแตะช่องที่อยากไป | ลูกศร/WASD หรือคลิกช่องที่อยากไป |
| วางตัวลงสนาม | แตะตัวในทีม แล้วแตะช่องบนสนาม | คลิก หรือกดเลข 1–6 แล้วคลิก |
| ย้ายตัวที่วางไปแล้ว | เลือกตัวนั้น แล้วกดปุ่ม **ย้ายตำแหน่ง** แล้วแตะช่องใหม่ | ปุ่มเดียวกัน หรือกด `V` |

ปุ่มทิศทางมีเพราะหน้าที่ฝังใน iframe คีย์บอร์ดมักไม่ทำงานจนกว่าจะคลิกในกรอบก่อน
และบนแท็บเล็ตก็ไม่มีอะไรบอกว่าเดินยังไง

## บัญชีผู้เล่นและหน้าผู้ดูแล

เปิดเกมครั้งแรกจะให้ตั้งชื่อผู้เล่น **คนแรกที่สมัครในเครื่องนั้นได้สิทธิ์ผู้ดูแล**
คนอื่นสมัครเพิ่มได้ แยกเซฟคนละชุด สลับกันเล่นได้ด้วยปุ่มชื่อย่อมุมขวาบน

> รหัสผ่านเก็บเป็นแฮช (SHA-256 + เกลือรายคน) ไม่ได้เก็บตัวจริง
> แต่นี่เป็นระบบฝั่งเบราว์เซอร์ล้วน ๆ — คนที่เปิด devtools ได้ก็แก้ข้อมูลในเครื่องตัวเองได้อยู่ดี
> เหมาะกับการกันคนในบ้านเผลอเล่นทับเซฟกัน ไม่ใช่ระบบความปลอดภัยจริง

**หน้าผู้ดูแล** (ปุ่ม *ผู้ดูแล* บนแถบบน) มีสองแท็บ:

- **ค่าเกม** — ปรับตัวเลขที่คุมเกมได้สด ๆ มีผลทันทีและจำไว้ข้ามการรีโหลด
  ราคาลงสนาม · ค่าหัวศัตรู · เงินคืนตอนเก็บกลับ · ตัวคูณเลือดศัตรู · ตัวคูณดาเมจ ·
  ความเร็วศัตรู · เวลาพักระหว่างเวฟ · เลเวลฟรีต่อด่าน · เลเวลสูงสุด · จำนวนแท่นวาง ·
  ขนาดทีม · โอกาสเจอตัวป่า · โอกาสจับ · ราคาของในร้าน · และตารางเลือดรายด่านทั้ง 10 ด่าน
  ทุกค่ามีปุ่มคืนค่าเดิมทีละตัวและคืนทั้งหมด
- **ผู้ใช้** — ดูและแก้ข้อมูลของทุกคน: เงิน ลูกบอล ด่านที่ผ่าน เลเวลของแต่ละตัวในกล่อง
  เพิ่ม/ลบโปเกม่อน (มีปุ่มให้ครบ 151 ตัว) เปลี่ยนชื่อ ตั้งรหัสใหม่ เปลี่ยนสิทธิ์
  ล้างเซฟ และลบผู้ใช้ (ลบ admin คนสุดท้ายไม่ได้)

### ต่อเซิร์ฟเวอร์ทีหลัง

ตอนนี้ทุกอย่างเก็บใน `localStorage` ของเครื่องนั้น ๆ แต่ `js/store.js` แยกชั้นที่เก็บข้อมูล
ออกมาแล้ว ถ้าอยากให้เซฟข้ามเครื่อง (เล่นคอมต่อไอแพด) เขียนเซิร์ฟเวอร์ที่มีสามเส้นทางนี้:
...
แล้วสลับที่เก็บตอนบูตบรรทัดเดียว โดยไม่ต้องแก้โค้ดเกม:

```js
PTD.store.use(PTD.store.remote('https://api.example.com', token));
```

ตัวที่เก็บแบบต่อเน็ตใช้ `localStorage` เป็นแคชอยู่แล้ว เน็ตหลุดก็ยังเล่นต่อได้
...
**โซนซาฟารี 14 แห่ง** แบ่งตามภูมิภาค — คันโต 6 · โจโต 4 · โฮเอ็น 4
โซนของแต่ละภูมิภาคเจอเฉพาะสายพันธุ์ของภูมิภาคนั้น
...
**ด่านแคมเปญ 18 ด่าน** แบ่งเป็นสามภูมิภาคละ 6 ด่าน บน 3 รูปทางเดิน × 10 ธีมสี
ศัตรูดึงจากถิ่นอาศัย **และช่วง id ของภูมิภาคนั้น** — ด่านโจโตจะไม่มีตัวคันโตโผล่มา
ผ่านแล้วได้เงิน ลูกบอล และหินเมก้า เลเวลที่ได้ระหว่างสู้ **ติดตัวไปด่านถัดไป**

**วางป้อมต้องจ่ายเงินและมีแท่นวางจำกัด** (16 แท่นต่อแผนที่ แบบ Kingdom Rush)
เงินตั้งต้นพอลงได้ 1–2 ตัว ที่เหลือต้องหาจากการฆ่า — ลำดับว่าจะเอาใครลงก่อนคือการตัดสินใจจริง
เก็บกลับได้เงินคืน 60% ย้ายตำแหน่งจึงมีต้นทุน

**เวฟมีลักษณะพิเศษ** สลับมาเรื่อย ๆ ทุกอันได้อย่างเสียอย่าง ไม่ใช่แค่ยากขึ้น:

| ลักษณะ | ผล |
|---|---|
| ฝูงเร็ว | เดินเร็วขึ้น 50% แต่เลือดเหลือ 72% |
| เกราะหนา | เกราะ ×2.1 แต่เดินช้าลง 22% |
| ฝูงใหญ่ | ตัวเยอะขึ้น 80% แต่เลือดเหลือ 52% |
| ฟื้นเลือด | ฟื้น 3.5%/วินาที ถ้าฆ่าไม่ขาด |

**เควสในตำนาน 21 ตัว** — คันโต 5 (Articuno…Mew) · โจโต 6 (Raikou…Celebi) ·
โฮเอ็น 10 (Regi สามตัว, Latias/Latios, Kyogre/Groudon/Rayquaza, Jirachi, Deoxys)
ปลดล็อกตามจำนวนด่านที่ผ่านและสายพันธุ์ที่จับได้ ตัวสุดท้าย (Deoxys) ต้องผ่านครบ 18 ด่าน
และจับให้ได้ 240 จาก 386 สายพันธุ์
...
js/store.js        ชั้นที่เก็บข้อมูล (localStorage ตอนนี้ / ต่อเซิร์ฟเวอร์ทีหลังได้)
js/auth.js         บัญชีผู้เล่น สิทธิ์ และเซฟแยกคน
js/config.js       ค่าที่หน้าผู้ดูแลแก้ได้ ทาทับค่าตั้งต้นตอนบูต
js/admin.js        หน้าผู้ดูแล
css/fonts.css      ประกาศฟอนต์ที่โฮสต์ไว้เอง (Silkscreen + Noto Sans Thai)
css/style.css      หน้าตาแบบเครื่องโปเกเด็กซ์ — ตัวแปรสีอยู่บน :root รองรับโหมดมืด
fonts/*.woff2      ไฟล์ฟอนต์ 7 ไฟล์ ~94 KB (สัญญาอนุญาต OFL)
js/types.js        ตารางธาตุ 18x18 + สีประจำธาตุ
js/dex.js          ข้อมูล Gen 1-3 ครบ 386 ตัว + ร่างเมก้า 43 ร่าง — สร้างอัตโนมัติ ห้ามแก้มือ
...
js/campaign.js     ตารางด่าน 18 ด่าน เควสในตำนาน 21 เควส และตารางเป้าหมายความยากรายด่าน
...
node tools/auth.js                        # ทดสอบระบบบัญชีและหน้าผู้ดูแลครบวงจร
node tools/balance.js best                # จำลองเล่นทั้งแคมเปญ ดูว่าบาลานซ์โอเคไหม
node tools/balance.js idle                # จำลองแบบ "วางแล้วปล่อยทิ้ง" — ต้องแพ้ตั้งแต่ด่าน 1
node tools/shots.js                       # เก็บภาพทุกหน้าจอไว้ดูว่าหน้าตาเพี้ยนตรงไหน
...
```
dist/index.html          395 KB  — CSS + JS 18 ไฟล์รวมอยู่ในนี้หมด
dist/fonts/*.woff2         7 ไฟล์ — ฟอนต์ ไม่ได้ดึงจาก Google Fonts
dist/sprites/still.json  470 KB  — ภาพนิ่ง 429 รูปเป็น data: URL ในไฟล์เดียว
dist/sprites/anim.b64.N.txt      — GIF เคลื่อนไหว 386 ตัว ต่อกันแล้วเข้ารหัส base64 หั่นเป็น 3 ชิ้น
dist/sprites/anim.json           — ดัชนีบอกว่ามีกี่ชิ้นและแต่ละตัวอยู่ตำแหน่งไหน
```

รวมทั้งหมด 13 ไฟล์ ~18 MB ซึ่งอยู่ในขีดจำกัดของที่เผยแพร่ (255 ไฟล์ · 16 MB ต่อไฟล์)

เหตุผลที่ต้องแพ็ก:
- **จำนวนไฟล์** — 386 ตัว + 43 ร่างเมก้า = 429 ไฟล์ เกินขีดจำกัด 255
- **ขนาดไฟล์** — GIF ทั้งหมด 13 MB พอเข้ารหัส base64 เป็น ~18 MB ซึ่งเกิน 16 MB ต่อไฟล์
  จึงต้องหั่นเป็นชิ้น แล้ว `js/sprites.js` ต่อกลับตอนโหลด
- **ชนิดไฟล์** — หลายที่เสิร์ฟเฉพาะชนิดไฟล์มาตรฐานของเว็บ `.bin` จึงใช้ไม่ได้ ต้องเป็น `.txt`

ภาพนิ่งรวมกันแค่ 340 KB จึงยัดเป็น `data:` URL ไปเลย ทำให้ `stillURL()` ยังคืนค่าแบบทันที
ไม่ต้องแก้ทุกจุดที่เอาไปใส่ `<img src>`
...
ฟอนต์ก็โฮสต์ไว้เองด้วยเหตุผลเดียวกัน — `tools/check-dist.js` ตรวจว่าหน้าที่รวมแล้ว
**ไม่เรียกโฮสต์ภายนอกเลยสักคำขอ**
...
| กลยุทธ์ | ผล |
|---|---|
| `best` (จัดทีมให้ธาตุครอบคลุม วางให้ถูกที่ เลี้ยงเลเวล) | ผ่านครบ 18 ด่าน |
| `random` (จัดทีมมั่ว ไม่คิดเรื่องธาตุ) | **ตายที่ด่าน 4** |
| `idle` (วางเท่าที่เงินตั้งต้นพอ แล้วไม่แตะอะไรอีก) | **ตายตั้งแต่ด่าน 1 เวฟ 7** |
| เควสในตำนาน 21 ตัว | ผ่านครบ ถ้าเก็บสายพันธุ์ขยันพอ (`node tools/balance.js best 26`) |

`idle` มีไว้เป็นตัวกันไม่ให้เกมกลับไปชนะได้ด้วยการวางทิ้ง ถ้าวันไหน `idle` ผ่านด่าน 1 ได้ แปลว่าง่ายเกินไปแล้ว
ส่วน `secNeeded` ในรายงานบอกว่าถ้าจะกันไม่ให้หลุดเลย ต้องมี DPS พอฆ่าทุกตัวในกี่วินาที
— ตัวเลขนี้ใช้หาว่าด่านไหนเกินกำลังทีมจริง ๆ
...
- **ปรับค่าส่วนใหญ่ไม่ต้องแก้โค้ด** — ใช้หน้าผู้ดูแลในเกมได้เลย
- **ปรับความยากรายด่าน** — `HP_TARGET` ใน `js/campaign.js` คือเลือดรวมทั้งด่านที่ตั้งเป้าไว้ตรง ๆ
  (ด่าน 1 = 27k → ด่าน 10 = 372k) ฟังก์ชัน `normalize()` จะคูณ `hpMul` ของทุกเวฟให้ได้ยอดนั้นพอดี
  เลยปรับความยากได้โดยไม่ต้องไปยุ่งกับสูตรสร้างเวฟ ส่วนจำนวนตัวต่อเวฟอยู่ที่ `countStep`
...
- **เพิ่ม Gen 4+** — `tools/gen-pokedex.js` มี `MAX_ID = 386` ขยับขึ้นได้เลย
  (สไปรท์เคลื่อนไหวชุด BW มีถึง id 649) แต่ต้องเติมรายการร่างเมก้ากับชื่อหินเอง
  และตรวจขนาดแพ็กสไปรท์ว่ายังไม่เกินขีดจำกัด
